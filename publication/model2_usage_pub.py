...
time_output = open('model2/anchor_0.9/time_anchor.txt', 'w+')
...
for i in x3:
    GREEDY_VALUE = i
    result_json_output = open('model2/anchor_0.9/pub_sb_result_('+str(ANCHOR_THRESHOLD_VALUE) + '_' + str(GREEDY_VALUE) + ').json', 'w+')
    start = time.time()
        ...
        with sess.as_default():
            with tf.device('/gpu:0'):
                ...
                for id_record_line in lines:
                    ...
                    blocks, anchors = doBlock5(line, KB, LABEL_DICT, threshold=ANCHOR_THRESHOLD_VALUE)
                    # print(blocks)
                    # print(anchors)
                    ...
                    greedy_label = greedy_labeling(re_anchors, softmax_loss, GREEDY_VALUE, LABEL_DICT)
                    ...
                    if len_Unknown2(greedy_labels, LABEL_DICT) and len(greedy_labels) >= len(LABEL_DICT):
                        temp_list = []
                        for r in do_blocking2(greedy_blocks, greedy_labels, len(LABEL_DICT), LABEL_DICT):
                            # print('result:', r)
                            print('---------------------------')
                            # print(r[0])

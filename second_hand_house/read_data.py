    ...
    try:
        ...
        with connection.cursor() as cursor2:
            sql = "SELECT 来源,标题,房源编号,发布时间, 租金,押付方式,户型, 租凭方式,房屋类型," \
                  "装修,面积,朝向,楼层,小区, 位置,配置,联系人,联系方式,公司,店面, 房源描述,URL FROM anjuke WHERE id = 10"
            cursor2.execute(sql)
            result = cursor2.fetchall()
            for row in result:
                if row[0] != '':
                    sources.append(' '.join(remove_black_space(jieba.lcut(row[0]))))
                if row[1] != '':
                    titles.append(' '.join(remove_black_space(jieba.lcut(row[1]))))
                if row[2] != '':
                    houseIDs.append(row[2])
                if row[3] != '':
                    publish_times.append(' '.join(remove_black_space(jieba.lcut(row[3]))))
                if row[4] != '':
                    rents.append(' '.join(remove_black_space(jieba.lcut(row[4]))))
                if row[5] != '':
                    charge_methods.append(' '.join(remove_black_space(jieba.lcut(row[5]))))
                if row[6] != '':
                    units.append(' '.join(remove_black_space(jieba.lcut(row[6]))))
                if row[7] != '':
                ...
                if row[10] != '':
                    areas.append(' '.join(remove_black_space(jieba.lcut(row[10]))))
                if row[11] != '':
                    orientations.append(row[11])
                if row[12] != '':
                    floors.append(' '.join(remove_black_space(jieba.lcut(row[12]))))
                if row[13] != '':
...
if __name__ == '__main__':
    ...
    print(x_text)
    # print(len(x_text))
    # s = '永利广场旁 国际公馆 精装三房 出行方便 看房随时 拎包入住'
    # print(jieba.lcut(s))
    # print(' '.join(remove_black_space(jieba.lcut(s))))

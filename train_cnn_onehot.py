...
import datetime
import numpy as np
from text_cnn_onehot import TextCNN_ONEHOT


class TrainCNN_ONEHOT(object):
    def __init__(self, vocab_size, embedding_dim, sequence_length, num_classes, filter_sizes=[1, 2, 3],
                 num_filters=50):
        g = tf.Graph()
        with g.as_default():
    ...
    def test_step(self, w_batch, y_batch, writer=None):
        feed_dict = {
            self.cnn.input_x: w_batch,
            self.cnn.input_y: y_batch,
            self.cnn.dropout_keep_prob: 0.5,     # set 0.5 at train step
    ...
    def cnn_train_onehot(self, w_tr, w_te, y_tr, y_te, test_every=200, checkpoint_every=200, batch_size=64, num_epoch=10,
                         shuffle=True):
        ...
        for epoch in range(num_epoch):
            ...
            for batch_num in range(num_batch_per_epoch):
                ...
                self.train_step(s_w_tr[start:end], s_y_tr[start:end])
                ...
                if current_step % test_every == 0:
                    ...
                    self.test_step(w_te[0:test_length], y_te[0:test_length], writer=self.dev_summary_writer)
                    print("")
